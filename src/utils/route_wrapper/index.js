...
function Wrapper({ element, history, match, routeMap, closeModal }) {
  ...
  const getParam = (param, alternative) => {
    return (
      (history && history.location && history.location.state && history.location.state[param]) ||
      alternative
    );
  };
...
const RoutesGenerator = ({ routeMap }) => {
  return Object.keys(routeMap).map(route => {
    ...
    return (
      <Route
        ...
        render={props => <Wrapper element={<Component />} {...props} routeMap={routeMap} />}
      />
    );
  });
};
